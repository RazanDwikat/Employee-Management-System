      ...
      meta: { requiresAuth: true, role: 'manager' },
       ...
       children: [
...

        {
          ...
          component: () => import('../views/manager/TeamAttendance.vue')
...
router.beforeEach((to, from, next) => {
  ...
  if (to.meta.requiresAuth && !authStore.isAuthenticated) {
    ...
    next('/dashboard')
  } else if (to.meta.role && !['manager', 'employee'].includes(authStore.user?.role) && to.meta.role === 'employee') {
    next('/dashboard')
  } else if (to.meta.role && authStore.user?.role !== to.meta.role && to.meta.role !== 'employee') {
    next('/dashboard')
  } else {
    next()
  }
